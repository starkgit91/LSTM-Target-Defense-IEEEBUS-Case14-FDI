...
class power_env(FDI):
    def __init__(self, case, case_name, noise_sigma, idx, fpr):
        ...
        # Load
        load_active_dir = np.genfromtxt("src/case14/load_active.csv",delimiter=',',dtype=np.str)
        load_reactive_dir = np.genfromtxt("src/case14/load_reactive.csv",delimiter=',',dtype=np.str)
        # load_active_dir = f"src/case14/load_active.csv"
        # load_active_dir = pd.read_csv(load_active_dir)
        # load_active_dir = load_active_dir.iloc[:,:].values
